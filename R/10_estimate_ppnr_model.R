...
# Script 10 — Estimate PPNR Model
# ============================================================
# Objective:
#   Estimate reduced-form PPNR models using the modelling sample
#   created in Script 08.
#
# Main input:
...
#
# Model families:
#   1. Scenario-only benchmark model
#   2. Pooled OLS with controls
#   3. Year fixed effects
#   4. Bank fixed effects
#   5. Bank and year fixed effects
#   6. Alternative PPNR rate model
#
# Main outputs:
#   data/processed/model/ppnr_model_estimation_sample.csv
#   data/processed/model/ppnr_model_predictions.csv
#   outputs/ppnr_model/script10_ppnr_model_report.docx
#   outputs/ppnr_model/script10_ppnr_model_outputs.xlsx
#   outputs/ppnr_model/script10_execution_log.txt
#
# Methodological note:
#   This script estimates public, reduced-form PPNR models.
...
cat("\n")
cat("============================================================\n")
cat("Starting Script 10 - Estimate PPNR Model\n")
cat("============================================================\n")

project_root <- "D:/GitHub/us-bank-stress-test-dfast-replication"
script_id <- "10"
script_name <- "estimate_ppnr_model"
start_time <- Sys.time()

setwd(project_root)

dir.create("data/processed/model", recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/ppnr_model", recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/ppnr_model/figures", recursive = TRUE, showWarnings = FALSE)
...
save_plot <- function(plot_object, file_name, width = 9, height = 5.5) {
  path <- file.path("outputs/ppnr_model/figures", file_name)
...
required_column_check_path <- "outputs/ppnr_model/script10_required_column_check.csv"
readr::write_csv(required_column_check, required_column_check_path)

if (length(missing_required) > 0) {
  cat("Missing required variables:\n")
  print(missing_required)
  stop("Script 10 stopped because required variables are missing.")
}
...
cat("Building PPNR estimation sample...\n")

estimation_sample <- sample_raw |>
  dplyr::filter(ppnr_model_sample == 1) |>
  dplyr::mutate(
...
if (nrow(estimation_sample) < 50) {
  stop("Estimation sample too small for Script 10 models.")
}
...
estimation_sample_summary <- tibble::tibble(
  ...
  ppnr_model_flag_rows = sum(sample_raw$ppnr_model_sample == 1, na.rm = TRUE),
  ...
  mean_ppnr_ratio = safe_mean(estimation_sample$y_ppnr_ratio_model),
  sd_ppnr_ratio = safe_sd(estimation_sample$y_ppnr_ratio_model),
  mean_ppnr_rate = safe_mean(estimation_sample$y_ppnr_rate_model),
  sd_ppnr_rate = safe_sd(estimation_sample$y_ppnr_rate_model)
) |>
  safe_df()
...
bank_distribution <- estimation_sample |>
  ...
  dplyr::summarise(
    ...
    mean_ppnr_ratio = safe_mean(y_ppnr_ratio_model),
    max_ppnr_ratio = max(y_ppnr_ratio_model, na.rm = TRUE),
    min_ppnr_ratio = min(y_ppnr_ratio_model, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::arrange(dplyr::desc(mean_ppnr_ratio)) |>
  safe_df()
...
cat("Estimating PPNR models...\n")

formula_m1 <- y_ppnr_ratio_model ~ scenario_factor

formula_m2 <- y_ppnr_ratio_model ~
  scenario_factor +
  x_initial_cet1_model +
  log_rwa_actual

formula_m3 <- y_ppnr_ratio_model ~
  scenario_factor +
  x_initial_cet1_model +
  log_rwa_actual +
  x_rwa_growth_model +
...
model_specs <- list(
  ...
  fit_lm_safe(formula_m6, estimation_sample, "M6_ppnr_rate_bank_year_fixed_effects")
)
...
if (length(estimated_models) == 0) {
  stop("No PPNR model was estimated successfully.")
}
...
prediction_long <- prediction_base |>
  tidyr::pivot_longer(
    ...
    names_to = "model_name",
    values_to = "predicted_ppnr_ratio"
  ) |>
  dplyr::mutate(
    model_name = stringr::str_remove(model_name, "^pred_"),
    residual_ppnr_ratio = y_ppnr_ratio_model - predicted_ppnr_ratio
  ) |>
  safe_df()
...
prediction_metrics <- prediction_long |>
  ...
  dplyr::summarise(
    observations = dplyr::n(),
    rmse = safe_rmse(y_ppnr_ratio_model, predicted_ppnr_ratio),
    mae = safe_mae(y_ppnr_ratio_model, predicted_ppnr_ratio),
    r2_outcome_prediction = safe_r2(y_ppnr_ratio_model, predicted_ppnr_ratio),
    mean_residual = safe_mean(residual_ppnr_ratio),
    sd_residual = safe_sd(residual_ppnr_ratio),
    .groups = "drop"
  ) |>
  dplyr::arrange(rmse) |>
  safe_df()
...
residual_diagnostics <- prediction_long |>
  ...
  dplyr::summarise(
    observations = dplyr::n(),
    mean_actual = safe_mean(y_ppnr_ratio_model),
    mean_predicted = safe_mean(predicted_ppnr_ratio),
    mean_residual = safe_mean(residual_ppnr_ratio),
    rmse = safe_rmse(y_ppnr_ratio_model, predicted_ppnr_ratio),
    mae = safe_mae(y_ppnr_ratio_model, predicted_ppnr_ratio),
    .groups = "drop"
...
p1 <- ggplot2::ggplot(
  plot_data_best,
  ggplot2::aes(x = y_ppnr_ratio_model, y = predicted_ppnr_ratio)
) +
  ggplot2::geom_point(alpha = 0.6) +
  ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  ggplot2::labs(
    title = paste("Actual versus Predicted PPNR Ratio -", best_model_name),
    x = "Actual PPNR ratio",
...
figure_paths <- c(
  figure_paths,
  save_plot(p1, "fig01_actual_vs_predicted_ppnr_ratio.png")
)

p2 <- ggplot2::ggplot(
  plot_data_best,
  ggplot2::aes(x = residual_ppnr_ratio)
) +
...
p3 <- prediction_metrics |>
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(
    title = "PPNR Model Comparison by RMSE",
    x = "Model",
    y = "RMSE"
  ) +
  ggplot2::theme_minimal()
...
estimation_sample_path <- "data/processed/model/ppnr_model_estimation_sample.csv"
predictions_path <- "data/processed/model/ppnr_model_predictions.csv"
...
out_dir <- "outputs/ppnr_model"

estimation_sample_summary_path <- file.path(out_dir, "script10_estimation_sample_summary.csv")
scenario_distribution_path <- file.path(out_dir, "script10_scenario_distribution.csv")
bank_distribution_path <- file.path(out_dir, "script10_bank_distribution.csv")
model_status_path <- file.path(out_dir, "script10_model_status.csv")
coefficient_table_path <- file.path(out_dir, "script10_coefficient_table.csv")
fit_table_path <- file.path(out_dir, "script10_fit_table.csv")
prediction_metrics_path <- file.path(out_dir, "script10_prediction_metrics.csv")
scenario_effects_path <- file.path(out_dir, "script10_scenario_effects.csv")
residual_diagnostics_path <- file.path(out_dir, "script10_residual_diagnostics.csv")
bank_residual_ranking_path <- file.path(out_dir, "script10_bank_residual_ranking.csv")
figure_inventory_path <- file.path(out_dir, "script10_figure_inventory.csv")
execution_summary_path <- file.path(out_dir, "script10_execution_summary.csv")
excel_output_path <- file.path(out_dir, "script10_ppnr_model_outputs.xlsx")
report_docx_path <- file.path(out_dir, "script10_ppnr_model_report.docx")
execution_log_path <- file.path(out_dir, "script10_execution_log.txt")
...
doc <- doc |>
  officer::body_add_par("Script 10 - Estimate PPNR Model", style = "heading 1") |>
  ...
  officer::body_add_par(
    "This script estimates reduced-form public PPNR models using the modelling sample prepared in Script 08. The models explain observed DFAST PPNR ratios using scenario indicators, initial capital, RWA scale, RWA dynamics and fixed effects.",
    style = "Normal"
  ) |>
...
doc <- doc |>
  ...
  officer::body_add_par(
    "The estimated models are reduced-form and based only on public DFAST information. They should be interpreted as benchmarking and replication tools, not as replicas of confidential supervisory PPNR models.",
    style = "Normal"
  )
...
log_lines <- c(
  "============================================================",
  "Script 10 - Estimate PPNR Model completed",
  "============================================================",
...
cat("\n")
cat("============================================================\n")
cat("Script 10 - Estimate PPNR Model completed\n")
cat("============================================================\n")
cat("Project root:\n", project_root, "\n\n")
